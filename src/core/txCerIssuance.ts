...
function buildAuthorityUrl(path: string, authorityBaseUrl?: string): string {
    const base = String(authorityBaseUrl || '').trim();
    return base ? buildApiUrl(base, path) : path;
}
...
function normalizeGroupAuthority(groupID: string, rawValue: unknown): GroupAuthorityInfo {
    const response = (rawValue && typeof rawValue === 'object' ? rawValue : {}) as Record<string, any>;
    const raw = (response.data && typeof response.data === 'object' ? response.data : response) as Record<string, any>;
    return {
        groupID: String(raw.GroupID || raw.groupID || raw.group_id || groupID),
        aggrNodeID: String(raw.AggrID || raw.aggrID || raw.aggr_id || ''),
        assignNodeID: String(raw.AssiID || raw.assignID || raw.assign_id || ''),
        aggrPublicKey: raw.AggrPublicKeyNew || raw.aggrPublicKeyNew || raw.aggr_public_key,
        assignPublicKey: raw.AssignPublicKeyNew || raw.assignPublicKeyNew || raw.assign_public_key,
        certifiers: raw.Certifiers || raw.certifiers,
    };
}

async function fetchGroupAuthority(groupID: string, authorityBaseUrl?: string): Promise<GroupAuthorityInfo> {
    if (authorityBaseUrl) {
        const response = await apiClient.get<unknown>(
            buildAuthorityUrl(API_ENDPOINTS.GROUP_INFO(groupID), authorityBaseUrl),
            { silent: true, useBigIntParsing: true },
        );
        return normalizeGroupAuthority(groupID, response);
    }
    const response = await getGroupInfo(groupID);
    if (!response.success || !response.data) throw new Error(`group authority unavailable: ${groupID}`);
    return normalizeGroupAuthority(groupID, response.data);
}

export function buildTXCerAuthoritySnapshot(
    record: TXCerIssuanceRecordV2,
    sourceGroup: GroupAuthorityInfo,
    targetGroup: GroupAuthorityInfo,
    certifiers: CertifierInfo[],
    capturedAt = Date.now(),
    ...
    if (!sourceGroup.aggrPublicKey || !sourceGroup.assignPublicKey || !targetGroup.assignPublicKey) throw new Error('group authority public keys are incomplete');
    const publicKeys: Record<string, PublicKeyV2> = { aggr: sourceGroup.aggrPublicKey };
    if (sourceGroup.aggrNodeID) publicKeys[sourceGroup.aggrNodeID] = sourceGroup.aggrPublicKey;
    if (sourceGroup.assignNodeID) publicKeys[sourceGroup.assignNodeID] = sourceGroup.assignPublicKey;
    if (targetGroup.assignNodeID) publicKeys[targetGroup.assignNodeID] = targetGroup.assignPublicKey;
...
export function buildVerifiedTXCerIssuanceMetadata(
    ...
    const proofStatus = security.cfaaAuditStatus === 'Verified' ? 'verified' : security.cfaaAuditStatus === 'Failed' ? 'invalid' : security.cfaaAuditStatus === 'Unavailable' ? 'unsupported' : 'missingProof';
...
export async function refreshTXCerIssuanceMetadata(
    ...
    try {
        ...
        incoming.security = evaluateTXCerSecurity({ issuanceRecord: protocolRecord, fastEvidence: protocolRecord.FastEvidence, assignAck: protocolRecord.Ack, liabilityReceipt: protocolRecord.LiabilityReceipt, issuanceProof: detail.proof || detail.Proof }, lifecycleStatus);
