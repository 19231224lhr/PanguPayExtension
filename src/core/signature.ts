...
export function bigIntToHex(value: bigint | string | number | null | undefined, padLength = 64): string {
...
export function generate8DigitFromInputHex(hex: string): string {
    const normalized = String(hex || '').replace(/^0x/i, '').toLowerCase().replace(/^0+/, '');
...
export function generateAccountIdFromPrivate(privateKeyHex: string): string {
    return generate8DigitFromInputHex(privateKeyHex);
}
