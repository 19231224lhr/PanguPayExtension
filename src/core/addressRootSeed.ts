...
import { bytesToHex, hexToBytes } from './signature';
...
function hmacSha256Bytes(keyBytes: Uint8Array, messageBytes: Uint8Array): Uint8Array {
    ...
    return Uint8Array.from(hexToBytes(digest.toString(CryptoJS.enc.Hex)));
}
...
export function formatAddressRootSeedForExport(rootSeedHex: string, addressType?: number): string {
    ...
    if (label) return `${ADDRESS_ROOT_SEED_PREFIX}${label}_${normalized}`;
...
export function derivePrivateKeyHexFromAddressRootSeed(rootSeedHex: string, addressType = 0): string {
    const normalized = String(rootSeedHex || '').replace(/^0x/i, '').toLowerCase();
    if (!/^[0-9a-f]{64}$/.test(normalized)) {
        throw new Error('Address root seed must be 64 hex characters');
    }

    const keyBytes = Uint8Array.from(hexToBytes(normalized));
...
export function deriveAddressKeypairFromAddressRootSeed(rootSeedHex: string, addressType = 0): {
    address: string;
    privHex: string;
    pubXHex: string;
    pubYHex: string;
