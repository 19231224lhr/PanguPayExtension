...
export function buildSeedChain(masterSeed: ArrayLike<number>, chainLength = DefaultSeedChainLength): number[][] {
    const normalizedSeed = Array.from(masterSeed);
    if (normalizedSeed.length === 0) throw new Error('masterSeed is empty');
    if (chainLength <= 0) throw new Error('chainLength must be > 0');
...
export function deterministicMasterSeedFromPrivateKey(privateKeyHex: string, generation = 0): number[] {
    if (generation < 0) throw new Error(`invalid generation ${generation}`);
...
export function newDeterministicSeedChainStateFromPrivateKey(
    ...
    chainLength = DefaultSeedChainLength,
    generation = 0
): SeedChainState {
...
export function recoverDeterministicSeedChainStateFromPrivateKey(
    ...
    maxGeneration = DefaultSeedRecoverGenerationWindow
): SeedChainState {
...
export function nextAnchor(state: SeedChainState): number[] {
    if (state.step <= 0) throw new Error('seed chain exhausted');
...
export function needReAnchor(state: SeedChainState, threshold = DefaultSeedReAnchorThreshold): boolean {
