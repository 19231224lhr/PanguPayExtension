...
import { AlgorithmECDSAP256, getTimestamp, signHashEnvelope } from './signature';
...
export function attachSettlementAuths(transaction: Transaction, accountPrivateKeyHex: string): void {
  if (!transaction.TXInputsCertificate || transaction.TXInputsCertificate.length === 0) return;
  transaction.TXInputsCertificate = transaction.TXInputsCertificate.map(txCer => {
    const baseTXCer = { ...txCer, SettlementAuth: zeroSettlementAuth() };
    return { ...baseTXCer, SettlementAuth: buildSettlementAuth(transaction, baseTXCer, accountPrivateKeyHex) };
  });
}
