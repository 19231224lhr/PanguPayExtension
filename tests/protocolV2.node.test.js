...
async function loadProtocolV2() {
  const result = await esbuild.build({
    stdin: {
      contents: `import * as protocol from './src/protocol-v2/index.ts'; import { serializeForBackend } from './src/core/signature.ts'; globalThis.__protocolV2 = { ...protocol, serializeForBackend };`,
      resolveDir: root,
...
test('legacy client adapters delegate signing hashes to protocol-v2', () => {
  const txBuilderSource = fs.readFileSync(path.join(root, 'src', 'core', 'txBuilder.ts'), 'utf8');
  const settlementSource = fs.readFileSync(path.join(root, 'src', 'core', 'settlementAuth.ts'), 'utf8');
...
test('protocol-v2 derives independent spendability, fast-evidence and CFAA states', async () => {
  ...
  assert.equal(protocol.isTXCerLocallySpendable(verified), true);
  ...
  tampered.liabilityReceipt.OutstandingAfter = '1';
...
test('protocol-v2 evidence persistence merge is idempotent and preserves complete records', async () => {
  const protocol = await loadProtocolV2();
  const { evidence } = loadGolden();
  const existing = {
