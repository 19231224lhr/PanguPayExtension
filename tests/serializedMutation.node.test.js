...
async function loadMutationModule() {
  const result = await esbuild.build({
    stdin: {
      contents: `import * as mutation from './src/core/serializedMutation.ts'; globalThis.__mutation = mutation;`,
      resolveDir: root,
...
test('named serialized locks prevent a stale session writer from overtaking an account switch', async () => {
  const { withSerializedLock } = await loadMutationModule();
  assert.equal(typeof withSerializedLock, 'function');
  ...
  const staleRefresh = withSerializedLock('pangupay-session', async () => {
  ...
  const accountSwitch = withSerializedLock('pangupay-session', async () => {
    order.push('switch');
  });
  releaseRefresh();
  await Promise.all([staleRefresh, accountSwitch]);
